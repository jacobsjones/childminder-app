...
export async function generateInvoicePDF(invoiceData) {
    const { child, monthName, settings, sessions = [] } = invoiceData;

    // Postgres DECIMAL columns arrive as strings — coerce everything numeric
    const totalHours = Number(invoiceData.totalHours) || 0;
    const rate = Number(invoiceData.rate) || 0;
    const totalCost = Number(invoiceData.totalCost) || 0;
    ...
    // Fresh Garden colors
    const leaf = rgb(61 / 255, 133 / 255, 88 / 255);
    const leafDeep = rgb(42 / 255, 90 / 255, 64 / 255);
    const leafSoft = rgb(223 / 255, 240 / 255, 226 / 255);
    const honey = rgb(242 / 255, 223 / 255, 158 / 255);
    const darkGray = rgb(43 / 255, 58 / 255, 49 / 255);
    const lightGray = rgb(92 / 255, 110 / 255, 99 / 255);
    ...
    // Soft hill strip under the header
    page.drawRectangle({
        x: 0,
        y: height - 112,
        width: width,
    ...
    if (sessions && sessions.length > 0) {
        ...
        // Header text on top of the leaf band
        page.drawText('Date', {
            x: 50,
            y: yPosition,
            size: 10,
